...
export default function Regions() {
  return (
    <div className="mx-auto max-w-6xl px-5 py-10">
      <h1 className="font-display font-black text-3xl mb-1">找地方</h1>
      <p className="text-ink/60 text-sm mb-8 max-w-xl">
      ...
      */}
      <div className="tile-floor border border-ink/10 rounded-sm h-56 sm:h-72 flex items-center justify-center mb-10">
        <p className="font-mono text-xs text-ink/40 text-center px-4">
          [ 地圖插槽 · MAP SLOT ]
          <br />
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {regionClusters.map((r) => (
          <article key={r.id} className="tile-card p-4">
            <p className="font-mono text-xs text-ink/50">{r.county}</p>
            <h3 className="font-display font-bold text-lg mt-1">
              <NeonLink to={`/find?county=${encodeURIComponent(r.county)}`}>
                {r.clusterName}
              </NeonLink>
            </h3>
            <p className="text-sm text-ink/70 mt-2 leading-relaxed">{r.description}</p>
          </article>
        ))}
      </div>
    </div>
